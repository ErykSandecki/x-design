---
name: x-design-i18n
description: How translations work in x-design — i18next + react-i18next, flat dot-key JSON, language detection order, and where to add a new string. Load before hardcoding user-facing text in a component, adding a new translation key, or touching translations/.
---
...
Follows [[x-design-module-structure]]:
...
`store/appInitializer/saga.ts`). x-draft was explicitly asked for **system-language** detection
...
Same shape as [[x-design-theming]]'s `useTheme` (stored choice → system signal → hardcoded default)
