---
name: x-design-routing
description: How routing works in x-design — react-router v8, the named route registry, the guard/ProtectedRoute mechanism, and per-route document titles. Load before adding a new route or page, touching anything under core/Routing, or implementing route protection (auth-style redirects).
---
...
2. Add the translation key(s) for its title to `translations/resources/en.json` and `pl.json` (see
   [[x-design-i18n]]) — e.g. `"routing.title.about": "About"`.
3. Add an entry to `APP_ROUTES_DATA` (`constants/appRoutesData.ts`):
   ...
   ```
4. Build the page under `pages/AboutPage/` per [[x-design-component-structure]].
