---
name: x-design-scss-bem
description: BEM nesting convention for x-design's .module.scss files — sub-elements of a component go under `&__element` (kebab-case) inside the root class instead of separate flat top-level classes. Load before adding a second class to any .module.scss file, or reviewing one that already has more than one top-level selector.
---

# x-design SCSS BEM Convention
...
Modifiers follow the same pattern with `&--modifier` (not used yet anywhere in x-draft, but this is
...
x-draft's CSS Modules have **no** `camelCaseOnly` `localsConvention` configured, so the exported JS
...
[[x-design-component-structure]] — the root class still matches the component name exactly; BEM
nesting only governs classes *below* that root.
[[x-design-import-order]] / [[x-design-module-structure]] — unrelated axes (import grouping, file
