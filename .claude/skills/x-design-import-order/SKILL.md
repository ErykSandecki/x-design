---
name: x-design-import-order
description: Import statement ordering convention for x-design, mirrored from x-design. Load before writing or reviewing the import block of any .ts/.tsx file — covers external-package ordering, the `// category` comment groups, alphabetical group ordering, and default-vs-named import precedence.
---

# x-design Import Order
...
(see [[x-design-module-structure]] for why `enums.ts` is its own file, separate from `types.ts` and
