---
name: x-design-translation-namespace
...
# x-design Translation Namespace Convention
...
`translations/resources/en.json` / `pl.json` ([[x-design-i18n]]), never hardcoded in a `constants.ts`
...
reusable feature root — the page is just a thin route wrapper around it ([[x-design-routing]]).

x-draft mirrors this: `pages/DesignPage/` renders `components/Design/Canvas` and
...
[[x-design-i18n]] — the flat dot-key JSON convention the resulting keys are added to, and the
`t()`/`useTranslation()` wiring.
[[x-design-module-structure]] — `constants.ts` already holds other runtime constants; namespace
