...
import { TPage, TPageBuilderState, TReducerHistory, TSelectedElements } from 'store/pageBuilder/types';

export const areaHasTheSameCoordinates = (prevState: TReducerHistory, nextState: TPage): boolean => {
...
export const hasTheSameSelectedElements = (
  ...
  if (prevSelectedElements.length === nextSelectedElements.length) {
    ...
    return nextSelectedElements.every(({ id: nextId }) => prevId.includes(nextId));
  }

  return false;
};

export const parentHasNotChanged = (prevState: TReducerHistory, nextState: TPage): boolean =>
  prevState.selectedElements[0].parentId === nextState.selectedElements[0].parentId;

export const isRepeatedStateInHistory = (state: TPageBuilderState, type: TAction['type']): boolean => {
