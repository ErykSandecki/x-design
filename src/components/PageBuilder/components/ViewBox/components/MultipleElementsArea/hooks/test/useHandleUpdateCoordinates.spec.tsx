...
describe('useHandleUpdateCoordinates', () => {
  beforeAll(() => {
    // mock
  ...
  it(`should setCoordinates`, () => {
    ...
    // before
    renderHook(() => useHandleUpdateCoordinates(false, mockCallBack), {
      wrapper: ({ children }) => {
        const Wrapper = getProviderWrapper(store);

        return (
          <Wrapper>
            <RefsProvider itemsRefs={sharedRefs} zoomContentRefHtml={zoomContent}>
              {children}
            </RefsProvider>
          </Wrapper>
        );
